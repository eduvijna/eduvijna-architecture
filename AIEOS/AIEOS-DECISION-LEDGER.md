...
`ADR-AIEOS-*` and Teacher OS `ADR-042`–`ADR-048` are distinct ID families. In particular, Teacher OS **ADR-047** (Outcome-first Prepare Tomorrow) is not the same decision as platform **ADR-AIEOS-047** (Production Workflow Plane Identity & Least-Privilege Contract). Teacher OS **ADR-048** (Review Queue owns approval) is not the same decision as platform **ADR-AIEOS-048** (First-Production App Runtime & OCI Delivery Contract).
...
| [ADR-AIEOS-048](../decisions/ADR-AIEOS-048-aieos-first-production-app-runtime-oci-delivery-contract.md) | AIEOS First-Production App Runtime & OCI Delivery Contract | Frozen / Approved | App Platform `blr`/`blr1`; dedicated VPC `aieos-prod-blr1`/`10.130.0.0/20`; two separate worker apps; Preview `apps-s-1vcpu-1gb-fixed` bounded exception; `eduvijna-registry/aieos-backend` immutable digest; workload-specific RUN_TIME SECRET Temporal keys; independent activation boundaries; distinct from Teacher OS ADR-048; does not authorize App/VPC/OCI/deploy mutation |
