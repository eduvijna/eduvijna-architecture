...
**Production workflow plane (architecture only):** [ADR-AIEOS-047](../decisions/ADR-AIEOS-047-aieos-production-workflow-plane-identity-least-privilege-contract.md) is **ARCHITECTURE FROZEN / APPROVED** (not implemented / provisioned / deployed / production-ready / activated). Temporal Cloud = first-production hosting baseline (specialization of ADR-AIEOS-026; ADR-026 body not rewritten). Connection mode = Temporal Cloud Namespace Endpoint (exact hostname = provisioning output). Namespaces = environment-isolated (not per-tenant). TLS + certificate verification required. Distinct Temporal Cloud Service Account + API key for WORKFLOW_DISPATCHER vs TEMPORAL_WORKER. Minimum stable provider RBAC = Account Read + target Namespace Write; Custom Roles not a first-production dependency. Current Content Review contract = `ContentReviewWorkflowV1` / `aieos.content.review` / `review_decision_recorded`. Dispatcher Temporal secret env = `AIEOS_WORKFLOW_DISPATCHER_TEMPORAL_API_KEY`; worker retains separate `AIEOS_TEMPORAL_API_KEY`. **WORKFLOW dispatcher Backend runtime = NOT YET AUTHORIZED.** **Temporal Cloud production provisioning = NOT AUTHORIZED.** **Production execution = NOT AUTHORIZED.** Distinct from Teacher OS ADR-047.
