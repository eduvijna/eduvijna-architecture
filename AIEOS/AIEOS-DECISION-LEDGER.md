...
- **Commercial envelope (architecture):** Bootstrap operating target ≤ USD 240/month; hard ceiling USD 250/month. Architecture freeze does **not** authorize spend above the hard ceiling. Actual retained-estate + production composition may require a later exact commercial gate before provisioning.

Still open (not authorized / not frozen by this catalogue): production BlobStore adapter implementation; actual Asset HTTP implementation; backup worker implementation; backup schema/migrations; infrastructure repository/state authority; OpenTofu implementation/apply; production credentials; production schema-owner readiness; Asset production runtime composition; actual cloud-resource creation; cost/commercial gate for provisioning; production migration; production mutation; production deployment; PED-I03 Asset mutation activation; physical purge/retention/legal hold; Scale Production commercial purchase/entitlement execution.
