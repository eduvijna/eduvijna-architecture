...
`ADR-AIEOS-*` and Teacher OS `ADR-042`–`ADR-048` are distinct ID families. In particular, Teacher OS **ADR-047** (Outcome-first Prepare Tomorrow) is not the same decision as platform **ADR-AIEOS-047** (Production Workflow Plane Identity & Least-Privilege Contract).
