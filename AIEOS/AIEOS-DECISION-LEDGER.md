...
| ADR | Title | Status | Notes |
|-----|-------|--------|-------|
| [ADR-AIEOS-052](../decisions/ADR-AIEOS-052-aieos-preparation-kit-multi-artifact-generation-architecture.md) | AIEOS Preparation Kit & Multi-Artifact Generation Architecture | **Proposed / Pending Approval** | TOS-DEV04 Prepare Tomorrow; one structured `education.generate_preparation_kit` → six ContentVersions; provenance V2 `artifact_kind`; atomic Content UoW; rejects payload staging and `ai.generation_artifacts`; distinct from Teacher OS ADR-047; **does not authorize implementation** |
