...
`ADR-AIEOS-*` and Teacher OS `ADR-042`–`ADR-048` are distinct ID families. In particular, Teacher OS **ADR-047** (Outcome-first Prepare Tomorrow) is not the same decision as platform **ADR-AIEOS-047** (Production Workflow Plane Identity & Least-Privilege Contract). Teacher OS **ADR-048** (Review Queue owns approval) is not the same decision as platform **ADR-AIEOS-048** (First-Production App Runtime & OCI Delivery Contract), **ADR-AIEOS-048R1** (App Platform Provider-Compliant Naming Revision), **ADR-AIEOS-048R2** (App Platform Runtime Ownership Boundary Revision), **ADR-AIEOS-049** (App Platform State-Free Deployment Plane), **ADR-AIEOS-050** (App Platform Release Controller Implementation Architecture), or **ADR-AIEOS-051** (Backend Production OCI Build, Provenance & First-Publication Architecture).
...
| [ADR-AIEOS-025](../decisions/ADR-AIEOS-025-aieos-api-contract-integration-implementation-baseline.md) | AIEOS API Contract & Integration Implementation Baseline | Frozen / Approved | NATS JetStream; domain never publishes NATS directly; historical modular-first `AIEOS_EVENTS` / `aieos.event.v1.>` examples superseded for production by ADR-AIEOS-046 |
| [ADR-AIEOS-026](../decisions/ADR-AIEOS-026-aieos-workflow-implementation-baseline.md) | AIEOS Workflow Implementation Baseline | Frozen / Approved | Temporal; workflow state ≠ domain state; production hosting specialization deferred here and frozen by ADR-AIEOS-047 without rewriting this ADR |
...
| [ADR-AIEOS-045](../decisions/ADR-AIEOS-045-aieos-dispatcher-tenant-candidate-discovery-authority.md) | AIEOS Dispatcher Tenant-Candidate Discovery Authority | Frozen / Approved | Pending-work candidate discovery from outbox/intent queues; dedicated NOLOGIN NOBYPASSRLS candidate-reader identities; SECURITY DEFINER owned by candidate-reader not schema owner; no dispatcher BYPASSRLS; no cross-tenant payload visibility; does not authorize implementation or migration |
| [ADR-AIEOS-046](../decisions/ADR-AIEOS-046-aieos-production-event-plane-identity-least-privilege-contract.md) | AIEOS Production Event Plane Identity & Least-Privilege Contract | Frozen / Approved | HISTORICAL / BASE production EVENT plane contract; production stream `AIEOS_EVENTS_PROD` / `io.eduvijna.aieos.>`; EVENT PUB `io.eduvijna.aieos.content.>` superseded for current publisher-domain scope by ADR-AIEOS-046R1; SUB `_INBOX.>`; JWT+NKey `.creds` via `AIEOS_EVENT_DISPATCHER_NATS_CREDENTIALS`; separate `streamadmin`; EVENT runtime no `$JS.API` admin |
| [ADR-AIEOS-046R1](../decisions/ADR-AIEOS-046R1-aieos-production-event-plane-multi-domain-publisher-scope-revision.md) | AIEOS Production Event Plane Multi-Domain Publisher Scope Revision | Frozen / Approved | CURRENT EVENT publisher-domain-scope authority; Founder approved **2026-08-31**; closed PUB set `io.eduvijna.aieos.content.>` + `io.eduvijna.aieos.teaching.>`; no `io.eduvijna.aieos.>` publisher wildcard; narrow supersession of ADR-AIEOS-046 A46-INV-03 only; TeachingAssignment event family frozen; aligns with ADR-AIEOS-053; architecture only; **TOS-DEV06-I03 NOT AUTHORIZED** |
| [ADR-AIEOS-047](../decisions/ADR-AIEOS-047-aieos-production-workflow-plane-identity-least-privilege-contract.md) | AIEOS Production Workflow Plane Identity & Least-Privilege Contract | Frozen / Approved | Temporal Cloud first-production hosting; Namespace Endpoint; environment-isolated namespaces; TLS + cert verify; distinct WORKFLOW_DISPATCHER / TEMPORAL_WORKER service accounts + API keys; Account Read + Namespace Write; Custom Roles not required; ContentReviewWorkflowV1 / `aieos.content.review` / `review_decision_recorded`; dispatcher env `AIEOS_WORKFLOW_DISPATCHER_TEMPORAL_*`; worker `AIEOS_TEMPORAL_*` preserved; distinct from Teacher OS ADR-047; does not authorize Temporal provisioning or WORKFLOW dispatcher Backend runtime |
| [ADR-AIEOS-048](../decisions/ADR-AIEOS-048-aieos-first-production-app-runtime-oci-delivery-contract.md) | AIEOS First-Production App Runtime & OCI Delivery Contract | Frozen / Approved | HISTORICAL / BASE first-production App runtime topology/delivery contract; application names `eduvijna-aieos-prod-*` superseded for new creation by ADR-AIEOS-048R1; OpenTofu App ownership mechanism superseded by ADR-AIEOS-048R2; distinct from Teacher OS ADR-048; does not authorize App/VPC/OCI/deploy mutation |
| [ADR-AIEOS-048R1](../decisions/ADR-AIEOS-048R1-aieos-app-platform-provider-compliant-naming.md) | AIEOS App Platform Provider-Compliant Naming Revision | Frozen / Approved | CURRENT App Platform naming authority: `aieos-prod-workflow-dispatcher` (length 30) and `aieos-prod-temporal-worker` (length 26); naming only; does not authorize App/VPC/OCI/deploy mutation |
| [ADR-AIEOS-048R2](../decisions/ADR-AIEOS-048R2-aieos-app-platform-runtime-ownership-boundary.md) | AIEOS App Platform Runtime Ownership Boundary Revision | Frozen / Approved | CURRENT App Platform ownership-boundary authority; production OpenTofu `digitalocean_app` ownership REJECTED (provider 2.99.1 materializes out-of-band `EV[...]` secrets into plan/state); WPI-AP-SV01/R1 FORMALLY CLOSED — FAIL_OPEN_TOFU_SECRET_MATERIAL; requires governed state-free deployment plane (detailed by ADR-AIEOS-049); does not authorize deployment-plane implementation or production mutation |
| [ADR-AIEOS-049](../decisions/ADR-AIEOS-049-aieos-app-platform-state-free-deployment-plane.md) | AIEOS App Platform State-Free Deployment Plane | Frozen / Approved | CURRENT state-free App deployment-plane behavior authority (WPI-AP-DP01 design); direct DigitalOcean REST release controller; one App/process/release; transient in-memory secret/`EV[...]` handling; allowlist reconciliation; durable per-App lease + double-read fence; independent dispatcher/worker credentials/secrets; immutable OCI digest; native rollback; redacted evidence; mandatory disposable empirical validation before production; does not authorize implementation, live validation, credentials, or production execution |
| [ADR-AIEOS-050](../decisions/ADR-AIEOS-050-aieos-app-platform-release-controller-implementation-architecture.md) | AIEOS App Platform Release Controller Implementation Architecture | Frozen / Approved | CURRENT release-controller implementation architecture (WPI-AP-DP02 design); Infrastructure `tools/app_platform_release/`; Python 3.14 / `uv`; GitHub-hosted `ubuntu-24.04` only; two fixed `workflow_dispatch` workflows/Environments; Actions concurrency lease; direct `httpx` DO REST; zero automatic mutation retries; closed typed allowlist; four PAT classes; Environment secret delivery; `release-receipt.json` 90-day sink; offline `app-release-controller-validate`; WPI-AP-DP-TV01 required but paused on OCI digest per ADR-AIEOS-051; does not authorize implementation, credentials, live App CREATE, or production execution |
| [ADR-AIEOS-051](../decisions/ADR-AIEOS-051-aieos-backend-production-oci-build-provenance-first-publication-architecture.md) | AIEOS Backend Production OCI Build, Provenance & First-Publication Architecture | Frozen / Approved | CURRENT Backend production OCI architecture; one common `eduvijna-registry` / `aieos-backend` image for WORKFLOW_DISPATCHER + TEMPORAL_WORKER; exact clean Backend Git SHA; future `Dockerfile.backend-runtime`; fail-closed default command; sanitized provenance receipt; source-SHA tag convenience; immutable digest authority; WPI-OCI-I01 = source/offline only; WPI-OCI-P01 = NOT AUTHORIZED; WPI-AP-DP-TV01 = AUTHORIZED BUT PAUSED ON OCI MANIFEST DIGEST; does not authorize publication, credentials, App CREATE, or production deployment |
| [ADR-AIEOS-052](../decisions/ADR-AIEOS-052-aieos-preparation-kit-multi-artifact-generation-architecture.md) | AIEOS Preparation Kit & Multi-Artifact Generation Architecture | Frozen / Approved | CURRENT multi-artifact Prepare Tomorrow architecture (TOS-DEV04); `education.generate_preparation_kit`; provenance V2 `artifact_kind`; capability/revision-aware GenerationRun fences; atomic six-artifact Content UoW; deterministic Answer Key; DEV03/DEV04 coexistence; rejects payload staging and `ai.generation_artifacts`; distinct from Teacher OS ADR-047; architecture freeze ≠ DEV04 implementation |
| [ADR-AIEOS-053](../decisions/ADR-AIEOS-053-aieos-teaching-assignment-classroom-delivery-authority.md) | AIEOS Teaching Assignment & Classroom Delivery Authority | Frozen / Approved | TOS-DEV06; Founder approved **2026-08-31**; Publication ≠ Assignment; TeachingAssignment Teaching-domain intent SoR; exact ContentVersion bind + published_version_id precondition; ClassRef via AIEOS School Context façade; command Idempotency-Key (no teacher/version/class/due uniqueness); ACTIVE/CLOSED/CANCELLED; LMS delivery deferred; clarifies ADR-046 / ADR-AIEOS-052 delivery wording only; architecture freeze ≠ implementation; **DEV06-I01+ NOT AUTHORIZED** |

### Proposed / freeze candidates

| ADR | Title | Status | Summary |
|-----|-------|--------|---------|
| [ADR-AIEOS-054](../decisions/ADR-AIEOS-054-aieos-teaching-execution-observation-authority.md) | AIEOS Teaching Execution & Observation Authority | Proposed / Freeze Candidate | TOS-DEV07; Chief Architect accepted **2026-09-01**; HYBRID / Option D; TeachingExecution Teaching-domain execution SoR; Assigned ≠ Taught; immutable ContentVersion bindings; PRIVATE_EXECUTION_NOTE + CLASS_OBSERVATION only; no timetable dependency; execution lifecycle IN_PROGRESS/COMPLETED/CANCELLED; events `teaching.execution.started/completed/cancelled.v1`; no ADR-AIEOS-046R2; **Founder freeze PENDING**; **DEV07-I01+ NOT AUTHORIZED** |
...
- **Production OpenTofu backend:** **INITIALIZED** (OpenTofu 1.12.5; S3; `use_lockfile=true`); remote tfstate object = **MATERIALIZED / AUTHORITATIVE** (key `environments/production/opentofu.tfstate`); Stages 1–3B control-plane closure retained; **WPI-A01** Temporal Cloud first-production apply = **EXECUTED / FORMALLY CLOSED** (Namespace + two workload service accounts in state); normal App Platform / VPC / AIStor / NATS / Managed PostgreSQL workload plan/apply and further unauthorized mutation = **NOT AUTHORIZED**
...
**Production event plane (architecture only):** [ADR-AIEOS-046](../decisions/ADR-AIEOS-046-aieos-production-event-plane-identity-least-privilege-contract.md) is **Frozen / Approved** as historical/base production EVENT plane contract. [ADR-AIEOS-046R1](../decisions/ADR-AIEOS-046R1-aieos-production-event-plane-multi-domain-publisher-scope-revision.md) is **Frozen / Approved** as **current** EVENT publisher-domain-scope authority (Founder approved **2026-08-31**). Production stream `AIEOS_EVENTS_PROD` with subjects `io.eduvijna.aieos.>`; EVENT publisher PUB closed set `io.eduvijna.aieos.content.>` + `io.eduvijna.aieos.teaching.>` (**NOT** platform-wide `io.eduvijna.aieos.>`); EVENT publisher SUB `_INBOX.>`; credential env `AIEOS_EVENT_DISPATCHER_NATS_CREDENTIALS`; auth JWT + NKey `.creds` / in-memory `user_jwt_cb` + `signature_cb`; stream administration via separate `streamadmin`; EVENT runtime `$JS.API` stream-admin authority = **NONE**. Historical ADR-025 modular-first examples (`AIEOS_EVENTS`, `aieos.event.v1.>`) are not current production authority. **PED-I11 Backend EVENT dispatcher runtime source = IMPLEMENTED / MERGED** (Backend `origin/main` evidence `8e837d2ef723db468e18b0405cb8bbc039efa8c2`). **EVENT production provisioning / activation / execution / deployment = NOT AUTHORIZED.** **TOS-DEV06-I03 = NOT AUTHORIZED.**

**Production workflow plane (architecture only):** [ADR-AIEOS-047](../decisions/ADR-AIEOS-047-aieos-production-workflow-plane-identity-least-privilege-contract.md) is **ARCHITECTURE FROZEN / APPROVED**. Temporal Cloud = first-production hosting baseline (specialization of ADR-AIEOS-026; ADR-026 body not rewritten). Connection mode = Temporal Cloud Namespace Endpoint. Namespaces = environment-isolated (not per-tenant). TLS + certificate verification required. Distinct Temporal Cloud Service Account + API key for WORKFLOW_DISPATCHER vs TEMPORAL_WORKER. Minimum stable provider RBAC = Account Read + target Namespace Write; Custom Roles not a first-production dependency. Current Content Review contract = `ContentReviewWorkflowV1` / `aieos.content.review` / `review_decision_recorded`. Dispatcher Temporal secret env = `AIEOS_WORKFLOW_DISPATCHER_TEMPORAL_API_KEY`; worker retains separate `AIEOS_TEMPORAL_API_KEY`. Distinct from Teacher OS ADR-047. **Current status (post-freeze):** WORKFLOW_DISPATCHER Backend runtime source (**PED-I12**) = **IMPLEMENTED / MERGED** (Backend `origin/main` `8f4dd172e6a0ba8b4ad944b0ae22060442356342`); Temporal Cloud first-production Namespace + two workload service accounts (**WPI-A01**) = **PROVISIONED / CONFORMED / FORMALLY CLOSED** (Namespace `eduvijna-aieos-prod.w97q1`); Temporal runtime API keys = **NOT YET ISSUED / NOT AUTHORIZED**; production WORKFLOW dispatcher / TEMPORAL_WORKER execution & App Platform deployment = **NOT AUTHORIZED**. Historical ADR-047 freeze date non-authorizations remain accurate as architecture-boundary statements and must not be read as denying later separately authorized PED-I12 / WPI-A01 outcomes.

**First-production App runtime & OCI delivery (architecture only):** [ADR-AIEOS-048](../decisions/ADR-AIEOS-048-aieos-first-production-app-runtime-oci-delivery-contract.md) is **ARCHITECTURE FROZEN / APPROVED** as the **historical/base** first-production App runtime topology/delivery contract. [ADR-AIEOS-048R1](../decisions/ADR-AIEOS-048R1-aieos-app-platform-provider-compliant-naming.md) is **ARCHITECTURE FROZEN / APPROVED** as the **CURRENT** App Platform naming authority. [ADR-AIEOS-048R2](../decisions/ADR-AIEOS-048R2-aieos-app-platform-runtime-ownership-boundary.md) is **ARCHITECTURE FROZEN / APPROVED** as the **CURRENT** App Platform ownership-boundary authority. [ADR-AIEOS-049](../decisions/ADR-AIEOS-049-aieos-app-platform-state-free-deployment-plane.md) is **ARCHITECTURE FROZEN / APPROVED** as the **CURRENT** state-free App deployment-plane behavior authority (WPI-AP-DP01). [ADR-AIEOS-050](../decisions/ADR-AIEOS-050-aieos-app-platform-release-controller-implementation-architecture.md) is **ARCHITECTURE FROZEN / APPROVED** as the **CURRENT** release-controller implementation architecture (WPI-AP-DP02). [ADR-AIEOS-051](../decisions/ADR-AIEOS-051-aieos-backend-production-oci-build-provenance-first-publication-architecture.md) is **ARCHITECTURE FROZEN / APPROVED** as the **CURRENT** Backend production OCI build / provenance / first-publication architecture. App Platform region `blr` / datacenter `blr1`; dedicated VPC `aieos-prod-blr1` / `10.130.0.0/20` (default-blr1 reuse forbidden); two separate worker applications `aieos-prod-workflow-dispatcher` and `aieos-prod-temporal-worker` (superseding historical `eduvijna-aieos-prod-*` names); Preview SKU `apps-s-1vcpu-1gb-fixed` bounded Founder/Chief acceptance; common immutable Backend OCI digest from `eduvijna-registry/aieos-backend`; workload-specific RUN_TIME SECRET Temporal keys; independent Infrastructure activation boundaries. **Production OpenTofu `digitalocean_app` ownership = REJECTED** (WPI-AP-SV01/R1 = FORMALLY CLOSED — FAIL_OPEN_TOFU_SECRET_MATERIAL; provider 2.99.1). Production App lifecycle owner = governed **state-free REST release controller** per ADR-AIEOS-049 behavior + ADR-AIEOS-050 implementation architecture. **WPI-AP-DP02 = DESIGN COMPLETE / FROZEN**; implementation = **NOT AUTHORIZED**; **WPI-AP-DP-TV01 = AUTHORIZED BUT PAUSED ON OCI MANIFEST DIGEST**; production App deployment = **NOT AUTHORIZED**. **Production Backend OCI architecture = DESIGN FROZEN** (ADR-AIEOS-051). **WPI-OCI-I01 = AUTHORIZED SOURCE / OFFLINE IMPLEMENTATION ONLY** (after ADR-AIEOS-051 merge). **WPI-OCI-P01 live first publication = NOT AUTHORIZED**. Infrastructure **WPI-AP-I02** = **FORMALLY CLOSED** at `main` `157f25a6a580d01be92ce798594302cdfe84cc9f` (OpenTofu App ownership removed). Distinct from Teacher OS ADR-048. **App Platform creation/update / VPC creation / DOCR repository / OCI publication / secret injection / OpenTofu App plan-apply / production deployment = NOT AUTHORIZED.**

Still open (not authorized by this catalogue): production WORKFLOW dispatcher execution/deployment; TEMPORAL_WORKER execution/deployment; production Temporal runtime API-key issuance/injection; dedicated production VPC creation; App Platform application creation; governed production OCI build/publication/promotion ([ADR-AIEOS-051](../decisions/ADR-AIEOS-051-aieos-backend-production-oci-build-provenance-first-publication-architecture.md) Frozen / Approved — architecture only; WPI-OCI-I01 source/offline only after merge; WPI-OCI-P01 not authorized); App Platform runtime-secret injection ([ADR-AIEOS-048](../decisions/ADR-AIEOS-048-aieos-first-production-app-runtime-oci-delivery-contract.md) / [ADR-AIEOS-048R1](../decisions/ADR-AIEOS-048R1-aieos-app-platform-provider-compliant-naming.md) / [ADR-AIEOS-048R2](../decisions/ADR-AIEOS-048R2-aieos-app-platform-runtime-ownership-boundary.md) / [ADR-AIEOS-049](../decisions/ADR-AIEOS-049-aieos-app-platform-state-free-deployment-plane.md) / [ADR-AIEOS-050](../decisions/ADR-AIEOS-050-aieos-app-platform-release-controller-implementation-architecture.md) / [ADR-AIEOS-051](../decisions/ADR-AIEOS-051-aieos-backend-production-oci-build-provenance-first-publication-architecture.md) Frozen / Approved — architecture only); release-controller **implementation**, credential creation, and TV01 App CREATE (WPI-AP-DP02 design frozen by ADR-AIEOS-050; WPI-AP-DP-TV01 authorized but paused on OCI digest); EVENT dispatcher production activation / provisioning ([ADR-AIEOS-045](../decisions/ADR-AIEOS-045-aieos-dispatcher-tenant-candidate-discovery-authority.md) / [ADR-AIEOS-046](../decisions/ADR-AIEOS-046-aieos-production-event-plane-identity-least-privilege-contract.md) Frozen / Approved — architecture only; PED-I11 source = IMPLEMENTED / MERGED; production activation / provisioning / migration / deployment not authorized by catalogue); database candidate-function migration; production candidate-reader role provisioning; scheduled/reconciliation runtime ownership; production BlobStore runtime composition / production credential conformance; actual Asset HTTP implementation; backup worker implementation; backup schema/migrations; further production apply for App/VPC/AIStor/NATS/Managed PostgreSQL; production schema-owner readiness; Asset production runtime composition; production deployment; commercial release (≤250 or new Founder ceiling); production migration; production mutation; PED-I03 Asset mutation activation; physical purge/retention/legal hold; Scale Production commercial purchase/entitlement execution; DOKS retirement.
