...
- **Pre-apply execution (ADR-AIEOS-044):** NATS single-node Bootstrap; VPC `aieos-prod-blr1` / `10.130.0.0/20`; Spaces OpenTofu state bucket design; stable hostname→RFC1918 identity; Recovery Console admin; Smallstep offline CA; App Platform sizing implementation-gated
...
Still open (not authorized / not frozen by this catalogue): production BlobStore adapter implementation; actual Asset HTTP implementation; backup worker implementation; backup schema/migrations; production entrypoints / App Platform composition; OpenTofu apply; production credentials; production schema-owner readiness; Asset production runtime composition; actual cloud-resource creation; commercial release (≤250 or new Founder ceiling); production migration; production mutation; production deployment; PED-I03 Asset mutation activation; physical purge/retention/legal hold; Scale Production commercial purchase/entitlement execution; DOKS retirement.
