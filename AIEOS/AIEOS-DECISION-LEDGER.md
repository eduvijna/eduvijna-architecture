...
`ADR-AIEOS-*` and Teacher OS `ADR-042`–`ADR-048` are distinct ID families. In particular, Teacher OS **ADR-047** (Outcome-first Prepare Tomorrow) is not the same decision as platform **ADR-AIEOS-047** (Production Workflow Plane Identity & Least-Privilege Contract). Teacher OS **ADR-048** (Review Queue owns approval) is not the same decision as platform **ADR-AIEOS-048** (First-Production App Runtime & OCI Delivery Contract), **ADR-AIEOS-048R1** (App Platform Provider-Compliant Naming Revision), **ADR-AIEOS-048R2** (App Platform Runtime Ownership Boundary Revision), or **ADR-AIEOS-049** (App Platform State-Free Deployment Plane).
