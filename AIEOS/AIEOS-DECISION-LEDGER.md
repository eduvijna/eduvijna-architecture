...
| [ADR-AIEOS-046](../decisions/ADR-AIEOS-046-aieos-production-event-plane-identity-least-privilege-contract.md) | AIEOS Production Event Plane Identity & Least-Privilege Contract | Frozen / Approved | Production stream `AIEOS_EVENTS_PROD` / `io.eduvijna.aieos.>`; EVENT PUB `io.eduvijna.aieos.content.>`; SUB `_INBOX.>`; JWT+NKey `.creds` via `AIEOS_EVENT_DISPATCHER_NATS_CREDENTIALS` in-memory callbacks; separate `streamadmin`; EVENT runtime no `$JS.API` admin; supersedes ADR-025 modular-first `AIEOS_EVENTS` / `aieos.event.v1.>` for production; does not authorize provisioning or Backend daemon |
...
**Production event plane (architecture only):** [ADR-AIEOS-046](../decisions/ADR-AIEOS-046-aieos-production-event-plane-identity-least-privilege-contract.md) is **Frozen / Approved**. Production stream `AIEOS_EVENTS_PROD` with subjects `io.eduvijna.aieos.>`; EVENT publisher PUB `io.eduvijna.aieos.content.>`; EVENT publisher SUB `_INBOX.>`; credential env `AIEOS_EVENT_DISPATCHER_NATS_CREDENTIALS`; auth JWT + NKey `.creds` / in-memory `user_jwt_cb` + `signature_cb`; stream administration via separate `streamadmin`; EVENT runtime `$JS.API` stream-admin authority = **NONE**. Historical ADR-025 modular-first examples (`AIEOS_EVENTS`, `aieos.event.v1.>`) are not current production authority. **PED-I11 Backend EVENT dispatcher runtime source = IMPLEMENTED / MERGED** (Backend `origin/main` evidence `8e837d2ef723db468e18b0405cb8bbc039efa8c2`). **EVENT production provisioning / activation / execution / deployment = NOT AUTHORIZED.**
