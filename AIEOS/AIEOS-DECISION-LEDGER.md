...
| [ADR-AIEOS-044R2](../decisions/ADR-AIEOS-044R2-aieos-production-state-region-availability-resolution.md) | AIEOS Production State Region Availability Resolution | Frozen / Approved | Forward state authority = SFO3 / `eduvijna-aieos-tofu-state-prod-sfo3`; workload remains BLR1; Stage 1 bucket bootstrap + Stage 2 backend initialization completed; remote tfstate not materialized; no production apply authorization |
