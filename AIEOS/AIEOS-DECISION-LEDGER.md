...
| [ADR-AIEOS-044R2](../decisions/ADR-AIEOS-044R2-aieos-production-state-region-availability-resolution.md) | AIEOS Production State Region Availability Resolution | Frozen / Approved | Forward state → SFO3 / `eduvijna-aieos-tofu-state-prod-sfo3`; workload remains BLR1; Stage 1 suspended |
...
- **Production OpenTofu state region:** **SFO3** — intended bucket `eduvijna-aieos-tofu-state-prod-sfo3` (**NOT YET CREATED**); endpoint `https://sfo3.digitaloceanspaces.com`
- **Pre-apply execution (ADR-AIEOS-044 / 044R1 / 044R2):** NATS single-node Bootstrap; VPC `aieos-prod-blr1` / `10.130.0.0/20`; prior BLR1 state target `eduvijna-aieos-tofu-state-prod-blr1` = PLANNED / NEVER CREATED / SUPERSEDED; NYC3 Space `eduvijna-aieos-tofu-state-prod` = UNATTRIBUTED / PRE-EXISTING / NON-AUTHORITATIVE / HOLD; Stage 1 bootstrap execution **SUSPENDED** pending 044R2 merge, infrastructure SFO3 reconciliation, and fresh exact-source Stage 1 SFO3 authorization; stable hostname→RFC1918 identity; Recovery Console admin; Smallstep offline CA; App Platform sizing implementation-gated
