...
`ADR-AIEOS-*` and Teacher OS `ADR-042`–`ADR-048` are distinct ID families. In particular, Teacher OS **ADR-047** (Outcome-first Prepare Tomorrow) is not the same decision as platform **ADR-AIEOS-047** (Production Workflow Plane Identity & Least-Privilege Contract). Teacher OS **ADR-048** (Review Queue owns approval) is not the same decision as platform **ADR-AIEOS-048** (First-Production App Runtime & OCI Delivery Contract), **ADR-AIEOS-048R1** (App Platform Provider-Compliant Naming Revision), or **ADR-AIEOS-048R2** (App Platform Runtime Ownership Boundary Revision).
...
| [ADR-AIEOS-048R2](../decisions/ADR-AIEOS-048R2-aieos-app-platform-runtime-ownership-boundary.md) | AIEOS App Platform Runtime Ownership Boundary Revision | Frozen / Approved | CURRENT App Platform ownership/deployment authority; production OpenTofu `digitalocean_app` ownership REJECTED (provider 2.99.1 materializes out-of-band `EV[...]` secrets into plan/state); WPI-AP-SV01/R1 FORMALLY CLOSED — FAIL_OPEN_TOFU_SECRET_MATERIAL; governed state-free deployment plane required; Infrastructure App ownership SUPERSEDED — MUST REMAIN INACTIVE; does not authorize Infra correction, deployment-plane implementation, or production mutation |
