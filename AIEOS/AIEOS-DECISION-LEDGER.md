...
- **Pre-apply execution (ADR-AIEOS-044 / 044R1):** NATS single-node Bootstrap; VPC `aieos-prod-blr1` / `10.130.0.0/20`; intended Spaces OpenTofu state bucket `eduvijna-aieos-tofu-state-prod-blr1` (BLR1); NYC3 Space `eduvijna-aieos-tofu-state-prod` = UNATTRIBUTED / PRE-EXISTING / NON-AUTHORITATIVE / HOLD; Stage 1 bootstrap execution **SUSPENDED**; stable hostname→RFC1918 identity; Recovery Console admin; Smallstep offline CA; App Platform sizing implementation-gated
