...
The AIEOS platform ADR family is now canonically deposited under `decisions/ADR-AIEOS-*`. Teacher OS `ADR-042`–`ADR-048` remain a distinct ID family. Teacher OS **ADR-047** (Outcome-first Prepare Tomorrow) is not platform **ADR-AIEOS-047** (Production Workflow Plane Identity & Least-Privilege Contract). Teacher OS **ADR-048** (Review Queue owns approval) is not platform **ADR-AIEOS-048** (First-Production App Runtime & OCI Delivery Contract), **ADR-AIEOS-048R1** (App Platform Provider-Compliant Naming Revision), **ADR-AIEOS-048R2** (App Platform Runtime Ownership Boundary Revision), **ADR-AIEOS-049** (App Platform State-Free Deployment Plane), **ADR-AIEOS-050** (App Platform Release Controller Implementation Architecture), or **ADR-AIEOS-051** (Backend Production OCI Build, Provenance & First-Publication Architecture).
...
**Current workflow-plane implementation / provisioning status (post-ADR-047):**
...
| Production Backend OCI architecture | **DESIGN FROZEN** by ADR-AIEOS-051 |
| WPI-OCI-I01 | **AUTHORIZED SOURCE / OFFLINE IMPLEMENTATION ONLY** (after ADR-AIEOS-051 merge) |
...
| **TOS-DEV10** — Teacher OS Development Complete | **ACTIVE** | TOS-DEV10A readiness audit **COMPLETE / ACCEPTED** (classification: Architecture + Implementation required). [ADR-AIEOS-057](../decisions/ADR-AIEOS-057-aieos-teacher-os-development-complete-experience-authority.md) **PROPOSED / FREEZE CANDIDATE** (Founder freeze **not** granted). Library / Teacher Memory / AI Assistant implementation **not** authorized by ADR-057 alone. Development Ready ≠ Production Ready. |
...
**TOS-DEV08 Assessment authority (Frozen / Approved — IMPLEMENTATION COMPLETE):** [ADR-AIEOS-055](../decisions/ADR-AIEOS-055-aieos-assessment-learning-evidence-authority.md) remains **Frozen / Approved** (Founder / Product Architecture **2026-09-03**). Architecture decision status is distinct from implementation status. **TOS-DEV08 implementation COMPLETE** (DEV08-I01–I04 formally closed; real-stack Product E2E COMPLETE). Governed baseline: Backend `1fe28f4fd1a2a2070aa69d67daa49cd53ba5820d`; Frontend `30c94f3e0403b9a5a2e955c706766035490598f9`; Alembic `tosd080002`; OpenAPI SHA-256 `824B389D6D4EDB2EA5D8ED3A9E5411087B566DFDCA09C2AB0CD4FDED51C4D89D`. Implemented flow: TeachingExecution COMPLETED → **no** automatic ClassroomAssessment → teacher deliberately chooses Assess → ClassroomAssessment RECORD → RECORDED → reload / durable history → CORRECT under optimistic concurrency → optional VOID → VOIDED retained as terminal history. ClassroomAssessment remains **class-level only** (no `learner_id` / `student_id` / attempts / submissions / scores / grades / mastery / Assessment events / Temporal Assessment workflow / AI grading). Learner-specific assessment, mastery, Teacher Memory, Notification Center, Student Intelligence, and production activation remain **not authorized**.

**TOS-DEV09 Improve authority (Frozen / Approved — IMPLEMENTATION COMPLETE):** [ADR-AIEOS-056](../decisions/ADR-AIEOS-056-aieos-improve-remediation-authority.md) (v1.0.2) remains **Frozen / Approved** (Founder / Product Architecture **2026-09-04**; Chief Architect architecture review **ACCEPTED — 2026-09-04**). Architecture decision status is distinct from implementation status. **TOS-DEV09 implementation COMPLETE** (DEV09-I01–I04 formally closed; real-stack Product E2E COMPLETE). Governed baseline: Backend `62733e3ad0d48887f3cd1e1a4486839170a5d651`; Frontend `732c0b5f88b7342d27e6ee7f103cb1d182ed310b`; Alembic `tosd090002`. OPTION B retained: class-level Improve as Teaching application capability — teacher-deliberate `remediate_class` TeachingWork + immutable `TeachingWorkRemediationOrigin` (pinned Assessment revision **and** `source_class_result_level_snapshot`). Teacher-confirmed `goal_text` is the authoritative remediation generator instruction; Assessment note / Observation body are display-only and not Teaching-owned copies. Assessed ≠ Improvement required. No Improve SoR/lifecycle. Reuses Content → Review → Publish → Assign. Teacher Memory remains **out of DEV09** (addressed under TOS-DEV10 / ADR-AIEOS-057 Proposed). Historical ADR-AIEOS-056 body text is **not** rewritten by this orientation sync.

**TOS-DEV10 Development-Complete Experience authority (PROPOSED / FREEZE CANDIDATE):** [ADR-AIEOS-057](../decisions/ADR-AIEOS-057-aieos-teacher-os-development-complete-experience-authority.md) deposits Library v1 (Content façade; no Library SoR), Teacher Memory v1 (durable preferences SoR; ≠ Continuous Context), contextual AI Assistant v1 (AIEOS API only; READ/REASON/SUGGEST), Mission remediation presentation semantics (derived-on-read; ADR-AIEOS-056 retained), DEV04 bounded planning sufficiency, MCP-READY not MCP-EVERYTHING, and Educational Quality baseline sufficiency for Development Ready. **Founder freeze not granted.** Library / Memory / Assistant implementation remain **not authorized** by ADR-057 alone. TOS-DEV10A = **READINESS AUDIT COMPLETE**. TOS-DEV10 = **ACTIVE**. DEV10 implementation is **not** complete.
...
| Capability | Status |
|------------|--------|
| Teacher Memory (durable preferences / personalization) | **ADR-AIEOS-057 PROPOSED / FREEZE CANDIDATE** — not Frozen; implementation **not** authorized by ADR-057 alone |
| AI Assistant expansion | **ADR-AIEOS-057 PROPOSED / FREEZE CANDIDATE** — placeholder remains until authorized implementation; Founder freeze **not** granted |
...
| Teacher OS Development-Complete Experience (Library / Memory / Assistant / Mission presentation) | **ADR-AIEOS-057 PROPOSED / FREEZE CANDIDATE**; TOS-DEV10A COMPLETE; TOS-DEV10 ACTIVE; Founder freeze **not** granted; Library/Memory/Assistant implementation **not** authorized by ADR-057 alone |
...
| Assess complete ≠ mastery / Production Ready | ADR-AIEOS-055 remains Frozen / Approved; TOS-DEV08 COMPLETE; ADR-AIEOS-056 Frozen / Approved and **TOS-DEV09 COMPLETE**; TOS-DEV10 Development Ready still requires Library / Memory / Assistant under ADR-AIEOS-057 (Proposed); Teacher Memory implementation, Notification Center, Student Intelligence, and production activation remain **not authorized** as Production / out-of-scope work |
