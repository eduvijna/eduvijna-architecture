...
**Still OPEN (not frozen / not authorized by catalogue):** production BlobStore runtime composition / production credential conformance; actual Asset HTTP implementation; backup worker implementation; backup schema/migrations; production entrypoints / App Platform composition; OpenTofu apply; production credentials; production schema-owner readiness; Asset production runtime composition; actual cloud-resource creation (including the planned SFO3 state bucket `eduvijna-aieos-tofu-state-prod-sfo3`); commercial release (≤250 or new Founder ceiling); production migration; production mutation; production deployment; PED-I03 Asset mutation activation; physical purge/retention/legal hold; Scale Production commercial purchase/entitlement execution; DOKS retirement. Catalogue deposition authorizes none of: purchase, infrastructure apply, AIStor production install, production BlobStore composition / credential activation, Asset HTTP implementation, backup execution, restore, Stage 1 cloud bootstrap, or production mutation.
