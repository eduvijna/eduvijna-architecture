...
**TOS-DEV09 Improve authority (Proposed / Freeze Candidate — NOT IMPLEMENTED):** [ADR-AIEOS-056](../decisions/ADR-AIEOS-056-aieos-improve-remediation-authority.md) proposes OPTION B class-level Improve as Teaching application capability: teacher-deliberate `remediate_class` TeachingWork + immutable `TeachingWorkRemediationOrigin` (pinned Assessment revision). Assessed ≠ Improvement required. No Improve SoR/lifecycle. Reuses Content → Review → Publish → Assign. Teacher Memory is **out of DEV09**. **Founder freeze not granted.** **DEV09-I01+ is NOT AUTHORIZED.**
...
| Improve / class-level remediation (Assessed ≠ Improvement required) | **ADR-AIEOS-056 Proposed / Freeze Candidate** (TOS-DEV09P1); OPTION B `remediate_class` TeachingWork + immutable origin; `/teacher-os/improve` remains PlaceholderPage; **DEV09-I01+ NOT AUTHORIZED**; Teacher Memory out of DEV09 |
