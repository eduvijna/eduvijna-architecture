...
**Production infrastructure (architecture freeze only):** [ADR-AIEOS-037](../decisions/ADR-AIEOS-037-aieos-production-infrastructure-baseline.md) is Frozen / Approved (DigitalOcean production cloud baseline). [ADR-AIEOS-038](../decisions/ADR-AIEOS-038-aieos-cross-cloud-asset-object-storage-exception.md) is Frozen / Approved as a historical cross-cloud exception. [ADR-AIEOS-038R1](../decisions/ADR-AIEOS-038R1-aieos-digitalocean-only-asset-storage-direction.md) is Frozen / Approved as the current first-production Asset storage hosting direction (DigitalOcean-only). [ADR-AIEOS-039](../decisions/ADR-AIEOS-039-aieos-asset-blobstore-provider-selection.md) is Frozen / Approved as the first-production Asset BlobStore **software provider** (MinIO AIStor, AIEOS-operated). [ADR-AIEOS-040](../decisions/ADR-AIEOS-040-aieos-asset-blobstore-first-production-topology.md) is Frozen / Approved as the first-production Asset BlobStore **topology** (8 AIStor nodes × 1 dedicated DigitalOcean Volume/node, XFS, one erasure set N=8 EC:3, private BLR1 service, internal LB/TLS passthrough, fail-closed required mounts). DigitalOcean Spaces is **REJECTED**. Amazon S3 is **no longer advancing for first production**. AWS-BOOT-01 is cancelled / closed without implementation. PED-I10B7C-TV02 is cancelled before AWS resource creation. PED-I10B7E-TV04-R2 empirically validated the frozen topology in NON_PRODUCTION; raw validation artifacts are not canonical repository files.

**Still OPEN (not frozen / not authorized by catalogue):** production compute sizing; production Volume sizing/capacity; exact commercial purchase execution; object backup/DR; Asset maximum size; production BlobStore adapter; Asset HTTP; Asset events/outbox where applicable; schema-owner readiness; production runtime composition; PED-I03; production infrastructure apply; production mutation; production deployment. Catalogue deposition authorizes none of: purchase, infrastructure apply, AIStor production install, adapter, Asset HTTP, or production mutation.
