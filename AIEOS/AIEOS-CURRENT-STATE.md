...
| **TOS-DEV09** — Class-level Improve & Remediation | **Architecture Proposed / Freeze Candidate** — **NOT IMPLEMENTED** | [ADR-AIEOS-056](../decisions/ADR-AIEOS-056-aieos-improve-remediation-authority.md); TOS-DEV09P1 design + validation deposited; OPTION B TeachingWork + `remediate_class` + immutable origin; `/teacher-os/improve` remains PlaceholderPage; **DEV09-I01+ NOT AUTHORIZED** |
...
**TOS-DEV09 Improve authority (Proposed / Freeze Candidate — NOT IMPLEMENTED):** [ADR-AIEOS-056](../decisions/ADR-AIEOS-056-aieos-improve-remediation-authority.md) (v1.0.1) proposes OPTION B class-level Improve as Teaching application capability: teacher-deliberate `remediate_class` TeachingWork + immutable `TeachingWorkRemediationOrigin` (pinned Assessment revision **and** `source_class_result_level_snapshot`). Teacher-confirmed `goal_text` is the authoritative remediation generator instruction; Assessment note / Observation body are display-only and not Teaching-owned copies. Assessed ≠ Improvement required. No Improve SoR/lifecycle. Reuses Content → Review → Publish → Assign. Teacher Memory is **out of DEV09**. **Founder freeze not granted.** **DEV09-I01+ is NOT AUTHORIZED.**
...
| Improve / class-level remediation (Assessed ≠ Improvement required) | **ADR-AIEOS-056 Proposed / Freeze Candidate v1.0.1** (TOS-DEV09P1/P1R1); OPTION B `remediate_class` TeachingWork + immutable origin with result snapshot; `/teacher-os/improve` remains PlaceholderPage; **DEV09-I01+ NOT AUTHORIZED**; Teacher Memory out of DEV09 |
...
| Assess complete ≠ Improve / mastery authorized | ADR-AIEOS-055 remains Frozen / Approved; TOS-DEV08 class-level Assess is COMPLETE; ADR-AIEOS-056 Improve authority is **Proposed only** (not frozen; DEV09-I01+ not authorized); Teacher Memory, Notification Center, Student Intelligence, and production activation remain **not authorized** |
