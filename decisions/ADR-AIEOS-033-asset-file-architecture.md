...
**Catalogue note:** Chief Architect confirms this ADR is **Frozen / Approved**. Later current-use, mutation, and audit semantics belong to ADR-AIEOS-034 / 035 / 036 / 036R1. This ADR does **not** select a production BlobStore provider. Provider selection remains external to ADR-AIEOS-033. Later [ADR-AIEOS-038](ADR-AIEOS-038-aieos-cross-cloud-asset-object-storage-exception.md) allows a narrow cross-cloud managed Asset-storage exception without changing these BlobStore invariants. Later [ADR-AIEOS-039](ADR-AIEOS-039-aieos-asset-blobstore-provider-selection.md) selects MinIO AIStor as the first-production provider without changing these BlobStore invariants. Amazon S3 is not selected by this ADR.
